...
# This command ensures a clean installation.
# 1. Clean npm cache.
# 2. Remove old modules and lock file.
# 3. Install all dependencies from package.json (including devDependencies).
# 4. Force a rebuild of Electron to match the host system.
NPM_INSTALL_COMMAND="
  set -x
  ...
  echo '--- Forcing a rebuild of Electron ---'
  npm rebuild electron --force
  set +x
"
